...
public:
  ...
  Rectangle BBox(const Rectangle &other) const {
    auto ret =
        Rectangle(std::min(left, other.left), std::max(top, other.top),
                  std::max(right, other.right), std::min(bottom, other.bottom));
    return ret;
  }

  static std::optional<Rectangle> Intersect(const Rectangle &a,
                                            const Rectangle &b) {
  ...
  bool operator==(const Rectangle &other) const {
    if (left == other.left && right == other.right && top == other.top &&
        bottom == other.bottom) {
      return true;
    }
    return false;
  ...
  friend std::ostream &operator<<(std::ostream &cout,
                                  const std::optional<Rectangle> &rect) {
    if (!rect.has_value()) {
